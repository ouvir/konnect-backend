...
public class AttractionController {
    ...
    @Operation(
        ...
        - `area`: 지역명 (예: 서울, 전주, 기타 등)
        - `areaCode`: 지역 코드 (예: 1)
        - `curationTypeName`: 큐레이션 타입 이름 (예: Attractions, Stays, Shopping, Restaurants)
    """
    )
    @Parameters({
            @Parameter(name = "page", description = "페이지 번호 (0부터 시작)", example = "0"),
            @Parameter(name = "size", description = "한 페이지 당 항목 수", example = "10"),
            @Parameter(name = "title", description = "관광지 제목 키워드", example = "백년"),
            @Parameter(name = "area", description = "관광지 지역 키워드\n('기타' 입력 시, 서울/전주/부산/제주/여수 미포함)", example = "서울"),
            @Parameter(name = "areaCode", description = "관광지 지역 코드", example = "1"),
            @Parameter(name = "curationTypeName", description = "큐레이션 타입 이름", example = "Attractions")
    })
    @GetMapping
    public ResponseEntity<OffsetPage<AttractionDTO>> getAttractions(
            ...
            @RequestParam(required = false) String curationTypeName
    ) {
        SearchCondition condition = new SearchCondition();
        condition.put("title", title);
        condition.put("area", area);
        condition.put("areaCode", areaCode);
        condition.put("curationTypeName", curationTypeName);
